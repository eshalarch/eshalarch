...
export default function Header({ isDarkMode, setIsDarkMode, isLoggedIn, setIsLoggedIn, openAuth }) {
  ...
  return (
    <header className={`fixed top-0 left-0 w-full h-20 px-6 flex justify-between items-center z-50 shadow-lg backdrop-blur-md border-b transition-colors duration-300
      ...
      <div className="relative">
        ...
        {/* Dropdown Menu */}
        {isMenuOpen && (
          <div className={`absolute left-0 mt-3 w-52 border rounded-xl shadow-2xl p-2 z-50 transition-all flex flex-col gap-1
            ...
            <div className="h-[1px] my-1 bg-zinc-800 dark:bg-zinc-800 light-theme:bg-zinc-200" />

            {/* Signup Login Dynamic Button */}
            <button 
              onClick={() => {
                setIsMenuOpen(false);
                if (isLoggedIn) {
                  setIsLoggedIn(false); // Signout Option
                } else {
                  openAuth();
                }
              }}
              className={`w-full flex items-center justify-between px-3 py-2 rounded-lg text-xs font-bold transition-all text-left
                ${isLoggedIn 
                  ? 'bg-[#148346]/20 text-[#148346]' 
                  : 'bg-[#c85a32]/10 text-[#c85a32] hover:bg-[#c85a32]/25'}`}
            >
              <span>{isLoggedIn ? 'Account Verified' : 'Signup / Login'}</span>
