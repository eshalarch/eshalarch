  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 text-white">
      <div className={`p-6 border rounded-2xl shadow-xl h-fit ${isDarkMode ? 'bg-[#111115] border-zinc-800' : 'bg-white border-zinc-200 text-black'}`}>
        <h2 className="text-sm font-bold tracking-widest mb-4 text-[#c85a32]">➕ ADD NEW PROJECT</h2>
        <div className="flex flex-col gap-3">
          ...
          <input className={`w-full border rounded-xl px-4 py-2.5 text-xs ${isDarkMode ? 'bg-black border-zinc-800' : 'bg-zinc-50 border-zinc-300'}`} placeholder="LOCATION" value={location} onChange={(e) => setLocation(e.target.value)} />
          <input className={`w-full border rounded-xl px-4 py-2.5 text-xs ${isDarkMode ? 'bg-black border-zinc-800' : 'bg-zinc-50 border-zinc-300'}`} placeholder="TAG" value={tag} onChange={(e) => setTag(e.target.value)} />
          <input className={`w-full border rounded-xl px-4 py-2.5 text-xs ${isDarkMode ? 'bg-black border-zinc-800' : 'bg-zinc-50 border-zinc-300'}`} placeholder="IMAGE URL" value={img} onChange={(e) => setImg(e.target.value)} />
          <button onClick={handleAddProject} disabled={loading} className="w-full py-3 bg-[#c85a32] text-white text-xs font-bold rounded-xl active:scale-95 transition-all">
          ...
          {(projectsData || []).map((project) => (
            <div key={project.id || Math.random()} className={`p-4 border rounded-xl flex items-center justify-between ${isDarkMode ? 'bg-[#111115] border-zinc-800' : 'bg-white border-zinc-200 text-black'}`}>
              <div>
                ...
                <p className="text-[10px] text-zinc-500">{project.location}</p>
              </div>
            </div>
          ))}
        </div>
