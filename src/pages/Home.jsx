  ...
  return (
    <div className="px-4 py-24 min-h-screen transition-colors duration-300">
      ...
      <div className="text-center mb-16">
        <h1 className={`text-3xl font-extrabold tracking-tight mb-3 transition-colors duration-300
      ...
      <div className="border-l-4 border-[#148346] pl-3 mb-8">
        <h2 className={`text-xl font-bold tracking-wide transition-colors duration-300
          ${isDarkMode ? 'text-white' : 'text-zinc-900'}`}>
          FEATURED PORTFOLIO
        </h2>
        <p className="text-[10px] text-zinc-500 font-mono mt-1">Our Recent Structural & Render Milestones</p>
      </div>

      {/* Portfolio Card List with strict text contrast rule */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        {portfolioItems.map((item) => (
          <div 
            key={item.id} 
            className={`border rounded-2xl overflow-hidden shadow-xl transition-all duration-300
              ${isDarkMode ? 'bg-[#111115] border-zinc-800/80' : 'bg-white border-zinc-200'}`}
          >
            {/* Image Box */}
            ...
            {/* Info Text Box - STRICT DARK/LIGHT TEXT RULES */}
            <div className="p-5">
