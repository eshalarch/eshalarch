...
export default function Services({ isDarkMode }) {
  ...
  return (
    <div className="px-4 py-24 min-h-screen transition-colors duration-300">
      ...
      {/* ASLI GRID VIEW */}
      {viewMode === 'grid' && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 w-full opacity-100 transition-all duration-300">
          {servicesData.map((service) => (
            <div 
              key={service.id}
              className={`p-6 border rounded-2xl shadow-xl transition-all duration-300 flex flex-col gap-2 ${service.color}
                ${isDarkMode ? 'bg-gradient-to-br from-zinc-900 to-black text-white' : 'bg-white border-zinc-200 text-zinc-900'}`}
            >
              <div className={`text-2xl font-bold ${service.numColor} leading-none`}>{service.id}</div>
              <div>
                <h3 className="text-base font-bold tracking-wide">{service.title}</h3>
      ...
      {/* ASLI LIST VIEW */}
      {viewMode === 'list' && (
        <div className="flex flex-col gap-4 max-w-xl mx-auto w-full opacity-100 transition-all duration-300">
          {servicesData.map((service) => (
            <div 
              key={service.id}
              className={`p-6 border rounded-2xl shadow-xl transition-all duration-300 flex flex-row gap-5 items-start ${service.color}
                ${isDarkMode ? 'bg-gradient-to-br from-zinc-900 to-black text-white' : 'bg-white border-zinc-200 text-zinc-900'}`}
            >
              <div className={`text-2xl font-bold ${service.numColor} leading-none pt-1`}>{service.id}</div>
              <div className="flex-1">
                <h3 className="text-base font-bold tracking-wide">{service.title}</h3>
                <p className={`text-xs mt-1 transition-all leading-relaxed ${isDarkMode ? 'text-zinc-400' : 'text-zinc-600'}`}>{service.desc}</p>
              </div>
            </div>
          ))}
        </div>
